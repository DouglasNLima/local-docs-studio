...
- **Word** creates a `.docx` with rendered diagrams and compatible content. Use **Import Word template** and the **Word template** selector to apply a browser-local `.docx` template pack during export. Use **Manage** beside the selector to list, select, rename, or delete imported templates; deleting the selected template returns to **No template / Default Word export**.
...
- Word export templates are imported from user-provided `.docx` files and stored locally in this browser profile. Export clones the template package, replaces its body, and preserves the selected content page master, semantic styles, numbering, theme, font table, settings, first/even/default headers and footers, fields, relationships, shapes, and referenced media. Markdown H1 supplies the document title; H2 maps to the first content-heading level. Compatible literal heading counters are omitted only when the mapped Word style supplies automatic numbering. Tables reuse a detected style or direct-formatting prototype, and oversized diagrams are fitted or split within the section margins.
- The Word template management dialog shows **No template / Default Word export** plus all imported packs with their selected state, original filename, and import date where available. Rename changes only the display name and keeps the stable template id and package data; delete requires confirmation, removes the complete self-contained local record, and repairs a deleted or stale selection to the default export path.
- Local development smoke ZIPs generated by `scripts/windows/New-LocalDevSmokeZip.ps1` can be served from a static server to manually validate Word template export without creating an official release.
- Windows local development smoke ZIPs generated by `scripts/windows/New-WindowsLocalDevSmokePackage.ps1` package `LensDocsStudio.Windows.exe` plus the updated `StaticApp/` so Word template export can be manually tested inside the Windows shell. They are local smoke artefacts only, not official releases, installers, release assets, frozen artefacts, publication bundles, or production-readiness claims.
- Corporate templates may be used only with permission and must stay out of source, generated ZIPs, Windows smoke packages, and release assets.
