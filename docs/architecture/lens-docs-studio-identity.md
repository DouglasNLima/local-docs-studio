...
Artefact bundle certification uses fixture-driven browser tests and documentation-only producer guidance without changing the app identity. The fixture catalogue proves valid, rich, invalid, unsafe, front matter, review pack, Docs Site, and generic ZIP paths while keeping `lens-artifact-bundle-1.0` optional. The producer guide and manual smoke checklist support future compatible tools and release validation, but they do not add direct Lens links, service calls, new persistence keys, or a Lens-family-only product promise.
