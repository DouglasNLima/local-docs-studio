...
- Phase 3BC records the post-`v0.1.0-dev.3` backlog and candidate `v0.1.0-dev.4` scope as documentation/planning only. Phase 3BD adds the implementation readiness plan for the possible `v0.1.0-dev.4` cycle, and Phase 3BE implements the first approved slice: explicit-user-action Diagnostics support bundle generation. `v0.1.0-dev.4` is not approved, has no committed release date, changes no release assets, and does not claim production readiness or go-live approval.
