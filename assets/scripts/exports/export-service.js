...
import { createWordDocument } from './word-document-exporter.js';
import {
  ...
  const {
    ...
    async function exportPreviewWord() {
      try {
        ...
        const wordDocument = await createWordDocument({
          ...content,
          fallbackTitle: getExportTitle(),
          templatePack,
        });
        downloadBlob(wordDocument, getWordExportName());
